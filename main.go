...
func setupRoutes(r *mux.Router, dbConn *db.DBConnection) {
	...
	r.Handle("/api/productos", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.AddProducto(dbConn))))).Methods("POST")
	r.Handle("/api/productos/{idproducto}", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.EditProducto(dbConn))))).Methods("PUT")
	...
	r.Handle("/api/pedidos/{id_pedido}/sucursal", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.AdminActualizarSucursalPedido(dbConn))))).Methods("PUT")
	r.Handle("/api/pedidos/{id_pedido}/estatus", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.AdminActualizarEstatusPedido(dbConn))))).Methods("PUT")
	r.Handle("/api/pedidos/{id_pedido}/descuento", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.AdminAplicarDescuentoPedido(dbConn))))).Methods("PUT")
	r.Handle("/api/pedidos/{id_pedido}/detalles", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.AdminActualizarDetallesPedido(dbConn))))).Methods("PUT")
	r.Handle("/api/pedidos/{id_pedido}", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.AdminGetPedidoByID(dbConn))))).Methods("GET")
	// ADMIN clientes
	r.Handle("/api/admin/clientes", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.GetAllUsuariosConTienda(dbConn))))).Methods("GET")
	// ADMIN usuarios
	r.Handle("/api/admin/usuarios", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.GetAllAdminUsuarios(dbConn))))).Methods("GET")
	r.Handle("/api/admin/usuarios", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.CreateAdminUsuario(dbConn))))).Methods("POST")
	r.Handle("/api/admin/usuarios", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.UpdateAdminUsuario(dbConn))))).Methods("PUT")
	r.Handle("/api/admin/usuarios", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.DeleteAdminUsuario(dbConn))))).Methods("DELETE")
	// ADMIN personalizaciones
	r.Handle("/api/admin/personalizar", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.AdminCreatePersonalizacion(dbConn))))).Methods("POST")
	r.Handle("/api/admin/personalizar/{id}", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.AdminUpdatePersonalizacionByID(dbConn))))).Methods("PUT")
	r.Handle("/api/admin/personalizar/{id}", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.AdminDeletePersonalizacionByID(dbConn))))).Methods("DELETE")
	r.Handle("/api/admin/personalizar", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.AdminGetAllPersonalizaciones(dbConn))))).Methods("GET")
	r.Handle("/api/admin/personalizar/{id}", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.AdminGetPersonalizacionByID(dbConn))))).Methods("GET")
	// Logo empresa edición (solo admin)
	r.Handle("/api/empresa/logo", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.EmpresaUploadLogo(dbConn))))).Methods("POST")
	r.Handle("/api/empresa/logo", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.EmpresaUpdateLogo(dbConn))))).Methods("PUT")
	r.Handle("/api/empresa/logo", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.EmpresaDeleteLogo(dbConn))))).Methods("DELETE")
	// Sincronización de pedidos
	r.Handle("/api/pedidos/sincronizar", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.SincronizarPedido(dbConn))))).Methods("POST")
	...
	r.Handle("/api/pedidos/actualizar_fecha_entrega", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.ActualizarFechaEntrega(dbConn))))).Methods("POST")
	...
	r.Handle("/api/admin/config-entrega", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.UpdateConfigEntrega(dbConn))))).Methods("POST")
	r.Handle("/api/admin/config-entrega", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.GetConfigEntrega(dbConn))))).Methods("GET")
	// Fechas de entrega
	...
	r.Handle("/api/sucursales/{id}/lista-precios", middlewares.RequireAdmin(middlewares.JWTAuthMiddleware(http.HandlerFunc(rutas.UpdateListaPreciosSucursal(dbConn))))).Methods("PUT")
